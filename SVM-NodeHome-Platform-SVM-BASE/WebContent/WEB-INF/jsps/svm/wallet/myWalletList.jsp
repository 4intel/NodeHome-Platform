...
<html>
  <head>
		...
		setTimeout(function () {
			 ...
			 if(sResult['result']=="OK" && sResult['list']!="" && sResult['list'].length>0) {
		    	...
		    	for(var i=0; i < walletList.length ; i ++) {
		    		...
		    		if(vBalance!="") {
			    		var wallerNm = AWI_getConfig(walletList[i]['walletId']);	

			    		var walletRegisted = false;
			    		var configChk = AWI_getConfig("REG_"+walletList[i]['walletId']);
			    		if(configChk=="") {
				    		var data = callJsonAPI("/svm/wallet/queryWalletInfo", {"serviceId":"<%=GlobalProperties.getProperty("project_serviceid")%>", "walletId":walletList[i]['walletId']} );
				    		if(data.result=="OK") {
				    			walletRegisted = true;
				    			AWI_setConfig("REG_"+walletList[i]['walletId'],"Y");	// Whether to register in the chain
				    		} else {
				    			AWI_setConfig("REG_"+walletList[i]['walletId'],"N");	// Whether to register in the chain
				    		}
			    		} else {
			    		...
			    		sHTML += '<td style="word-break: break-all;"><span style="font-size:16px; font-weight:bold; color:#f58903;">' + gfnAddComma(parseInt( vBalance )/<%=CoinUtil.DISPLAY_COIN_UNIT%>) + '</span> <span style="font-size:14px;" class="aaa"> <%=CoinListVO.getCoinCou()%></span></td>';
			    		sHTML += '<td>';
			    		if(walletRegisted)
	...
	function FnSelectWallet() {
		...
		if(selId!="") {
			if(confirm(selNm + ' <spring:message code="body.msg.select.wallet" />')) {
				AWI_setConfig("CUR_WID",selId);
	...
	function FnAddWallet() {
		...
		var regFee = parseInt("<%=CoinListVO.getWalletRegFee()%>");
		if(frm.name_WalletID && frm.name_WalletID.length>0) {
			for(var i=0; i<frm.name_WalletID.length; i++) {
				if(frm.name_WalletID[i].checked) {
					selId = frm.name_WalletID[i].value;
		...
		if(regFee > iWalletBalance) {
			alert('<spring:message code="wallet.msg.addwallet.fail3" /> <spring:message code="body.text.wallet.regist.fee" />(<%=CoinUtil.calcDisplayCoin(Double.parseDouble(CoinListVO.getWalletRegFee()))%> <%=CoinListVO.getCoinCou()%>)');
			return;
		}
		if(selId!="") {
			location.href="/svm/wallet/registWalletForm?sWid="+selId;			
		} else {
			alert('<spring:message code="body.msg.select.please.wallet" />');
		}
	}
	...
	function FnBlockUseWallet() {
		...
		var minRemitt = "<%=CoinListVO.getMinRemittanceAmount() %>";
		if(minRemitt <= iWalletBalance) {
			alert('<spring:message code="wallet.msg.stop.wallet.confirm2" />'+minRemitt+' <%=CoinListVO.getCoinCou()%>');
			return;
		}
		...
		if(confm) {
			if(selId!="") {
				...
				setTimeout(function () {
					...
					// ************ step2 : get Signature / S-T API
					sQuery = ["PID","10000",sNonce];
					var sigRes = AWI_getSignature(selId, sQuery);
					...
					if(sigRes['result']=="OK") {
						...
						retData = callJsonAPI("/svm/wallet/walletDisableRegist", sQuery);
