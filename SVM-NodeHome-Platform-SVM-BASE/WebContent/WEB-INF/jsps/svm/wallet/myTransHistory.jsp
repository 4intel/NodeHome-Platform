...
<html>
  <head>
	...
	function displayList(j_jaHistroy) {
		...
		for(var i = 0; i < j_jaHistroy.length ; i ++) {
			...
			if(gfnAddComma(j_jaHistroy[i]['flg'])=='1') {
				sHtml +="<span style=\"font-size:16px; font-weight:bold; color:#d34202;\">"+gfnAddComma(j_jaHistroy[i]['val']/<%=CoinUtil.DISPLAY_COIN_UNIT%>)+"</span> <span style=\"font-size:14px;\" class=\"aaa\"> <%=CoinListVO.getCoinCou()%></span><br />";
			} else {
				sHtml +="<span style=\"font-size:16px; font-weight:bold; color:blue;\">"+gfnAddComma(j_jaHistroy[i]['val']/<%=CoinUtil.DISPLAY_COIN_UNIT%>)+"</span> <span style=\"font-size:14px;\" class=\"aaa\"> <%=CoinListVO.getCoinCou()%></span><br />";
			}
			sHtml +="</td>";
			sHtml +="</tr>";
		}
	...
	function getNewHistory(pWalletId) {
		...
		sQuery = ["PID","10000",listCount+"",sNonce];
		var sigRes = AWI_getSignature(pWalletId, sQuery);
		if(sigRes['result']=="OK") {
			sSig = sigRes['signature_key'];	
			
			// ************ step3 : get TransHistory / SVM API
			sQuery = {"npid":sNpid, "parameterArgs" : ["PID","10000",listCount+"",sNonce,sSig,pWalletId]};
			retData = callJsonAPI("/svm/wallet/getNTransHistory", sQuery);	// Recent transaction history API calls
			if(retData['result'] == "OK") {
	...
	function getSearchHistory(pWalletId) {
		...
		if(sigRes['result']=="OK") {
			...
			retData = callJsonAPI("/svm/wallet/getTransHistory", sQuery);		// Transaction history API call
			if(retData['result'] == "OK") {
				return retData['value'];
			} else {
				return '';
